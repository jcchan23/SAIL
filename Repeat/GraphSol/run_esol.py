...
# dataset setting
parser.add_argument('--data_path', type=str, default='./data/preprocess/esol',
...
if __name__ == '__main__':
    ...
    # load total dataset
    with open(f'{args.data_path}/esol_train.pickle', 'rb') as f:
    ...
    if not os.path.exists(f'{args.result_path}/esol_seed_{args.seed}'):

        # build a folder
        os.makedirs(f'{args.result_path}/esol_seed_{args.seed}')
        ...
        for idx, (train_index, valid_index) in enumerate(kf.split(train_valid_names)):
            ...
            with open(f'{args.result_path}/esol_seed_{args.seed}/train_fold_{idx + 1}.txt', 'w') as fw:
                for name in train_names:
                    fw.write(name + '\n')
            with open(f'{args.result_path}/esol_seed_{args.seed}/valid_fold_{idx + 1}.txt', 'w') as fw:
                for name in valid_names:
                    fw.write(name + '\n')
    
    # load train and valid dataset
    assert args.run_fold > 0
    with open(f'{args.result_path}/esol_seed_{args.seed}/train_fold_{args.run_fold}.txt', 'r') as f:
        train_names = [line.strip() for line in f.readlines()]
    with open(f'{args.result_path}/esol_seed_{args.seed}/valid_fold_{args.run_fold}.txt', 'r') as f:
        valid_names = [line.strip() for line in f.readlines()]
    # load test dataset
    with open(f'{args.data_path}/esol_test.pickle', 'rb') as f:
        test_names, test_sequences, test_graphs, test_labels = pickle.load(f)
    ...
    test_loader, (_, _) = get_loader(test_names, test_sequences, test_graphs, test_labels,
                                     batch_size=args.batch_size, shuffle=False, num_workers=args.num_workers)
    ...
    # initital weight and print model summary, GraphSol not need initialize (why?)
    ...
    # log
    with open(f'{args.result_path}/esol_seed_{args.seed}/{model.__class__.__name__}_fold_{args.run_fold}.txt', 'w') as f:
        ...
        for epoch in range(1, args.max_epochs + 1):
            ...
            # train stage
            model.train()
            train_results = loop(train_loader, model, optimizer=optimizer, scheduler=scheduler, device=device)
            ...
            # store model
            if valid_results['r2'] > best_metric:
                best_epoch, best_metric, best_results = epoch, valid_results['r2'], valid_results
                torch.save(model.state_dict(), f'{args.result_path}/esol_seed_{args.seed}/{model.__class__.__name__}_fold_{args.run_fold}.ckpt')
        ...
        model.eval()
        with torch.no_grad():
            model.load_state_dict(torch.load(f'{args.result_path}/esol_seed_{args.seed}/{model.__class__.__name__}_fold_{args.run_fold}.ckpt', map_location=device))
            test_results = loop(test_loader, model, optimizer=None, scheduler=None, device=device)
