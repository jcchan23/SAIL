...
def build_dgl_graph(names, sequences, labels, data_path, result_path, dataset_name, mode):
    ...
    mean, std = np.load(f'{result_path}/features/train_mean.npy'), np.load(f'{result_path}/features/train_std.npy')
    ...
    for name, sequence, label in tqdm(zip(names, sequences, labels), total=len(names)):
        ...
        # [L, L]
        edge_features = np.load(f'{result_path}/features/edge_features/{name}.npy')
    ...
    # save graphs
    with open(f'{result_path}/{dataset_name}_{mode}.pickle', 'wb') as fw:
...
if __name__ == '__main__':
    ...
    data_path = './data/source/esol'
    result_path = './data/preprocess/esol'
    
    data = pd.read_csv(f'{data_path}/esol_train.csv', sep=',')
    names = data['gene'].values.tolist()
    sequences = data['sequence'].values.tolist()
    labels = data['solubility'].values.tolist()
    build_dgl_graph(names, sequences, labels, result_path, dataset_name="esol", mode="train")
    
    data = pd.read_csv(f'{data_path}/esol_test.csv',sep=',')
    names = data['gene'].values.tolist()
    sequences = data['sequence'].values.tolist()
    labels = data['solubility'].values.tolist()
