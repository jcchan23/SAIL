...
def loop(data_loader, model, optimizer, scheduler, loss_name, metric_name, data_mean, data_std, device):
    batch_size = data_loader.batch_size
    data_loader = tqdm(data_loader) if optimizer is not None else data_loader
    
    loss_sum, y_true, y_pred = 0.0, list(), list()
    for batch in data_loader:
        names, crystals, graphs, labels = batch
        ...
        # loss calculation
        loss = cal_loss(y_true=labels, y_pred=output, loss_name=loss_name, 
    ...
    metric_avg = cal_metric(y_true=np.array(y_true), y_pred=np.array(y_pred), metric_name=metric_name, 
                                 data_mean=data_mean.detach().cpu().numpy(), data_std=data_std.detach().cpu().numpy())
    
    return {'loss': loss_avg, f'{metric_name}': metric_avg}
...
def cal_loss(y_true, y_pred, loss_name, data_mean, data_std, device):
    ...
    if loss_name == 'mse':
        ...
        # convert labels to float
        y_true = y_true.float()
        loss = F.smooth_l1_loss(y_pred, y_true, reduction='sum') / y_true.shape[1]
    elif loss_name == 'bce':
