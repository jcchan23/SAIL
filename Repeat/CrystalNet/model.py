...
class CrystalNet(nn.Module):
    ...
    """
    def __init__(self, node_features, edge_features, hidden_features, output_features, num_step_message_passing=2):
    ...
    def forward(self, graphs, devices):
        ...
        graphs.ndata['i'] = self.node_emb(graphs.ndata['x'])
        
        # [batch_num_edges, edge_features] -> [batch_num_edges, hidden_features]
        graphs.edata['i'] = self.edge_emb(graphs.edata['x'])
        ...
        # Readout step: [batch_num_nodes, node_features] -> [batch_size, node_hiddens]
        batch_hiddens = self.pooling(graphs)
        # [batch_size, node_hidden_feats] -> [batch_size, output_features]
