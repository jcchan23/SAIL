...
if __name__ == '__main__':
    ...
    # load total dataset
    with open(f'{args.data_path}/densecpd/train.pickle', 'rb') as f:
    ...
    # cv split(if need!)
    if not os.path.exists(f'{args.result_path}/densecpd_seed_{args.seed}'):
        
        # build a folder
        os.makedirs(f'{args.result_path}/densecpd_seed_{args.seed}')
        ...
        for idx, (train_index, valid_index) in enumerate(kf.split(train_valid_names)):
            ...
            with open(f'{args.result_path}/densecpd_seed_{args.seed}/train_fold_{idx + 1}.txt', 'w') as tw:
                for pdb in train_names:
                    tw.write(pdb + '\n')
            with open(f'{args.result_path}/densecpd_seed_{args.seed}/valid_fold_{idx + 1}.txt', 'w') as vw:
                for pdb in valid_names:
                    vw.write(pdb + '\n')

    # load train and valid dataset
    assert args.run_fold > 0
    with open(f'{args.result_path}/densecpd_seed_{args.seed}/train_fold_{args.run_fold}.txt', 'r') as f:
        train_names = [line.strip() for line in f.readlines()]
    with open(f'{args.result_path}/densecpd_seed_{args.seed}/valid_fold_{args.run_fold}.txt', 'r') as f:
    ...
    with open(f'{args.result_path}/densecpd_seed_{args.seed}/{model.__class__.__name__}_fold_{args.run_fold}.txt', 'w') as f:
        ...
        for epoch in range(1, args.max_epochs + 1):
            ...
            if valid_results['accuracy'] > best_metric:
                ...
                torch.save(model.state_dict(), f'{args.result_path}/densecpd_seed_{args.seed}/{model.__class__.__name__}_fold_{args.run_fold}.ckpt')
