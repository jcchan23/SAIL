...
parser.add_argument('--run_fold', type=int, default=0, choices=[i for i in range(11)],
...
def test(dataset_name, args, device):
    ...
    # load test dataset
    with open(f'{args.data_path}/{dataset_name}/test.pickle', 'rb') as f:
    ...
    with open(f'{args.result_path}/{args.data_name}_seed_{args.seed}/{model.__class__.__name__}_{dataset_name}.txt', 'w') as f:
        if args.run_fold > 0:
            model_paths = glob(f'{args.result_path}/{args.data_name}_seed_{args.seed}/{model.__class__.__name__}_fold_{args.run_fold}.ckpt')
        else:
            model_paths = glob(f'{args.result_path}/{args.data_name}_seed_{args.seed}/{model.__class__.__name__}_fold_*.ckpt')
        ...
        data_frame.to_csv(f'{args.result_path}/{args.data_name}_seed_{args.seed}/{model.__class__.__name__}_{dataset_name}.csv', sep=',', index=None)
...
if __name__ == '__main__':
    ...
    # use args.data_name model to test all four datasets
