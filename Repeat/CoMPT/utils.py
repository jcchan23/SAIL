...
def loop(data_loader, model, optimizer, scheduler, loss_name, metric_name, data_mean, data_std, device):
    ...
    for idx, batch in enumerate(data_loader):
        ...
        # (batch, args.task_number)
        labels = labels.to(device)
        
        # (batch_size, args.task_number)
        output = model(batch_node_features, batch_edge_features, batch_distance_matrix, batch_mask, device)
    ...
    metric_avg = cal_metric(y_true=np.array(y_true), y_pred=np.array(y_pred), metric_name=metric_name, data_mean=data_mean.detach().cpu().numpy(), data_std=data_std.detach().cpu().numpy())
