...
if __name__ == '__main__':
    ...
    train_loader = get_loader(train_smiles, mols, graphs, labels, batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers)
    valid_loader = get_loader(valid_smiles, mols, graphs, labels, batch_size=args.batch_size, shuffle=False, num_workers=args.num_workers)
    test_loader = get_loader(test_smiles, mols, graphs, labels, batch_size=args.batch_size, shuffle=False, num_workers=args.num_workers)
    ...
    scheduler = NoamLR(optimizer=optimizer, warmup_epochs=[5], total_epochs=[args.max_epochs],steps_per_epoch=len(train_loader),
                       init_lr=[0], max_lr=[args.learning_rate], final_lr=[args.learning_rate])
